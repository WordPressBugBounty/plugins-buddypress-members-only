...
Copyright 2015 - 2025  Tomas Zhu  (https://membersonly.top)
...
//!!! start 3.4.9
$bprestrictrestapi = get_option('bprestrictrestapi');

if ($bprestrictrestapi == 'yes') {
    require_once("rules/rest.php");
}
//!!! end 3.4.9

//!!! before 3.4.9 require_once("rules/rest.php");
...
if (empty($bpmofreecurrentversion)) {
    ...
    define('BPMO_FREE_VERSION', '3.4.3');
...
function buddypress_members_only_setting()
{
    ...
    if (isset($_POST['bpmosubmitnew'])) {
        ...
        if (isset($_POST ['bprestrictrestapi'])) {
            //3.4.9
            $m_bprestrictrestapi = sanitize_text_field($_POST ['bprestrictrestapi']);
            update_option('bprestrictrestapi', $m_bprestrictrestapi);
        } else {
...
<tr style="margin-top:30px;">
										<td width="100%" style="padding: 20px;">
										<p>
										<?php echo __('<h3>Disable REST API:</h3>', 'bp-members-only'); ?>
										</p>
										<p>
										<?php
    $bprestrictrestapi = get_option('bprestrictrestapi');
    if (! (empty($bprestrictrestapi))) {
        echo '<input type="checkbox" id="bprestrictrestapi" name="bprestrictrestapi"  style="" value="yes"  checked="checked"> Disable REST API ';
    } else {
        echo '<input type="checkbox" id="bprestrictrestapi" name="bprestrictrestapi"  style="" value="yes" > Disable REST API ';
    }
    ?>
										</p>
										<p>
													<font color="Gray"><i>
										<?php
    echo __('# If you enable this option, the plugin will disable the REST API to enhance wordpress security', 'bp-members-only');
    ?></i>
    ...
    ?>
						<div id="dashboard-widgets-main-content">
							...
							<div class="inside">
									<ul>
									<li>
								* You can also download the latest Buddypress Members Only plugin for free from our <a href='https://membersonly.top/product/buddypress-members-only-free/'  target='_blank'>official download link</a> at any time.
								</li>
                                </ul>
						</div>									
	...
	<tbody>
		<tr class="form-field">
			<td><input name="bp_members_only_access_to_this_page" type="checkbox"
				value="yes"
				<?php  if (esc_attr($get_post_meta_value_for_this_page) == 'yes') {
				    echo 'checked="checked"';
				} ?>><label><?php _e('Allow everyone to access the page', 'admin-tools') ?></label>
			</td>
		</tr>
	</tbody>
</table>
...
function bpmo_user_first_run_guide_bar_free()
{
    $is_user_first_run_guide_bar = get_option('bpmo_user_first_run_guide_bar_free');
    if (empty($is_user_first_run_guide_bar)) {
        //!!! 3.5.9  wp_kses only allow white html tags
        $message = wp_kses(
            __('Thanks for installing <strong>BuddyPress Members Only</strong>! Please check <a href="' . esc_url(admin_url('admin.php?page=membersonlyfaq')) . '" target="_blank">Knowledge Base</a>, then set up via <a href="' . esc_url(admin_url('admin.php?page=bpmemberonly')) . '" target="_blank">Setting Panel</a>. Any question or feature request please contact <a href="https://membersonly.top/contact-us/" target="_blank">Support</a> :)', 'bp-members-only'),
            [
                'strong' => [],
                'a'      => ['href' => [], 'target' => []]
            ]
        );
        echo '<div class="notice bpmo-notice notice-info"><p>' . $message . '</p></div>';
        update_option('bpmo_user_first_run_guide_bar_free', 'yes');
    }
}
...
function bpmo_in_elementor_editor()
{
    if (isset($_POST['actions'])) {
        //!!! before 3.5.9 $check_elementor_editor = stripos($_POST['action'], 'elementor_ajax');
        //!!! 3.5.9
        $action = sanitize_text_field($_POST['action']);
        $check_elementor_editor = stripos($action, 'elementor_ajax');
...
function members_only_free_setting_panel_head($title)
{
    ...
    <div style="float:left;margin-right:10px;">
        <?php
        //!!! 3.5.9 image src use esc_url
        echo '<img src="' . esc_url(plugins_url('/images/new.png', __FILE__)) . '" style="width:30px;height:30px;" alt="" />';
    ?>
    </div>
    <div style="padding-top:5px;font-size:22px;"><?php echo esc_html($title); ?></div>
    <div style="clear:both;"></div>
    <?php
}
 ...
 function members_only_free_check_notify()
{
    ...
    $members_only_free_have_new_version = get_option('members_only_free_have_new_version_' . $update_version_json);
    if ($members_only_free_have_new_version == 'dismiss') {
        return;
    }
    else
    {
        members_only_free_only_update_notify($members_only_free_MessageString, $update_version_json);
    }
...
function members_only_free_check_notify()
{
    $update_version_json = '3.6.3';

    $msg  = '<p><b>' . esc_html__('BuddyPress Members Only', 'bp-members-only') . '</b> ' .
            esc_html__('has been migrated to a new WordPress repository address.', 'bp-members-only') . '</p>';
    $msg .= '<p>' . wp_kses(
        __('You can also download the latest free copy from <a href="https://membersonly.top/product/buddypress-members-only-free/" target="_blank">our official page</a>.', 'bp-members-only'),
        ['a' => ['href' => [], 'target' => []]]
    ) . '</p>';
    ...
    members_only_free_only_update_notify($msg, $update_version_json);
}

function members_only_free_hide_notices()
{
    if (isset($_GET['members-only-free-hide-notice']) && isset($_GET['members_only_free_notice_nonce'])) {
        //!!! 3.5.9
        if (!wp_verify_nonce(sanitize_text_field($_GET['members_only_free_notice_nonce']), 'members_only_free_hide_notices_nonce')) {
            wp_die(esc_html__('Action failed. Please refresh the page and retry.', 'members_only_free'));
        }
        if (!current_user_can('manage_options')) {
            wp_die(esc_html__('Cheatin&#8217; huh?', 'members_only_free'));
        }
        $hide_notice = sanitize_text_field($_GET['members-only-free-hide-notice']);
        update_option('members_only_free_have_new_version_' . $hide_notice, 'dismiss');
    }
}

//3.6.3
function members_only_free_only_update_notify($p_message, $version)
{
    echo '<div id="message" class="updated fade" style="position:relative;line-height:60px;margin-left:0;margin-top:10px;margin-bottom:10px;">';
    // 3.5.9 esc_html__
    echo '<a class="notice-dismiss members-only-free-update-dismiss" href="' .
         esc_url(wp_nonce_url(add_query_arg('members-only-free-hide-notice', $version), 'members_only_free_hide_notices_nonce', 'members_only_free_notice_nonce')) .
         '">' . esc_html__('Dismiss', 'bp-members-only') . '</a>';
    echo '<p>';
    echo $p_message;
    echo '</p></div>';
}

// add_action('admin_head', 'members_only_free_check_notify', 10);
// add_action('init',   'members_only_free_hide_notices');
