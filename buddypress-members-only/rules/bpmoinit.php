...
function bpmoinit()
{
    ...
    $bpmofreecurrentversion = sanitize_text_field((string) $bpmofreecurrentversion); //!!! 3.6.3
    $bpmofreecurrentversion = str_replace('.', '', $bpmofreecurrentversion);

    $inittime = time();

    if (!(empty($bpmofreecurrentversion))) {
        // before 3.6.3 $bpmoinitsetup = update_option ( 'bpmoinitsetup', $inittime );
        // !!! 3.6.3
        update_option('bpmoinitsetup', $inittime);
    } else {
        ...
        if (empty($bpmoinitsetup)) {
            ...
            if (empty($bpmoregisterpageurl)) {
                ...
                $bpmoregisterpageurl = esc_url($bpmoregisterpageurl); //!!! 3.6.3
            ...
            $bpopenedpageurl = esc_url($bpopenedpageurl); //!!! 3.6.3
